...
export default function OAuthPage() {
  useEffect(() => {
    ...
    const state = Math.random().toString(36).substring(2); // Simple state generator
    ...
    const authorizationUrl = `https://login.xero.com/identity/connect/authorize?response_type=code&client_id=${clientId}&redirect_uri=${encodeURIComponent(
      redirectUri
    )}&scope=${scopes}&state=${state}`; //&prompt=consent`; // Added prompt=consent
